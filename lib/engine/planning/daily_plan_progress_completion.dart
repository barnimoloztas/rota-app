...
import '../progress/progress_completion_lifecycle.dart';

class DailyPlanProgressCompletionResult {
  DailyPlanProgressCompletionResult._({
  ...
  /// Whether this call completed the planned Progress for the first time.
...
DailyPlanProgressCompletionResult completeDailyPlanProgress({
  ...
  if (plannedTask.type != StudyTaskType.progress) {
    throw StateError('The selected daily plan task is not a Progress.');
  }
  ...
  if (completedAcademicTaskIndexes.contains(academicTaskIndex)) {
    return DailyPlanProgressCompletionResult._(
  ...
  return DailyPlanProgressCompletionResult._(
    topicLifecycle: completeProgress(
